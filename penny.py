...
def get_live_price(asset, prices_dict):
    """Universal matching: Finds price for BTC, btc, Bitcoin, or bitcoin."""
    if not isinstance(prices_dict, dict): return None
    asset_upper = str(asset).upper().strip()
    
    # Check direct match (BTC or Bitcoin)
    if asset_upper in [k.upper() for k in prices_dict.keys()]:
        for k, v in prices_dict.items():
            if k.upper() == asset_upper: return float(v)
            
    # Check Cross-Reference (If ledger says BTC, check for Bitcoin in George's data)
    xr = {"BTC": "BITCOIN", "ETH": "ETHEREUM", "SOL": "SOLANA", "BITCOIN": "BTC"}
    target = xr.get(asset_upper)
    if target:
        for k, v in prices_dict.items():
            if k.upper() == target: return float(v)
    return None
...
def format_institutional_ledger(df, prices_dict):
    ...
    for _, row in df.iterrows():
        ...
        if res == 'OPEN' or res == 'ACTIVE':
            ...
            mtm = live_p if live_p else entry_p
