...
def get_firm_ledger(conn, prices_dict=None):
    """
    Penny 3.1: Cloud-Native Institutional Auditor.
    Now pulls 'Burn' data directly from the Google Sheets 'Overheads' tab.
    """
    default_data = {
    ...
    try:
        # STEP 1: Fetch Master Ledger
        df = conn.read(worksheet="Ledger", ttl="0")
        
        # STEP 2: Fetch Cloud Overheads (Replacing local overheads.csv)
        try:
            ...
            # If the Overheads tab is missing or empty, burn is zero
            burn = 0.0
        
        if df is None or df.empty:
            return {**default_data, "burn": burn}
        ...
        # STEP 4: Realized P/L Calculation
        ...
        # Vault Cash = Initial + Market Returns - Firm Burn
        vault_cash = float(INITIAL_CAPITAL + realized - burn)
        
        # Locked Capital (Currently in the market)
...
# --- [get_live_price, calculate_unrealized, format_institutional_ledger remain the same] ---
...
def get_live_price(asset, prices_dict):
    ...
    return clean_prices.get(target) if target else None
...
def format_institutional_ledger(df, prices_dict):
    ...
    for _, row in df_working.iterrows():
        ...
        report.append({
            "Ticker": asset_name, "Status": status, "Age": age_str,
            "Entry Price": round(entry_p, 2), "MTM Price": round(mtm, 2),
            "Return (%)": round(ret_pct, 2), "P/L ($)": round(pnl, 2)
        })
