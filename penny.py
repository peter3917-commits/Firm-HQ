...
def cleanup_legacy_trades(trades_df, prices_dict):
    """Penny's Housekeeping: Immune to index/column shifting."""
    if trades_df.empty:
        return trades_df

    # 1. FORCE CLEAN COLUMNS (Removes leading commas/indices)
    if 'Unnamed: 0' in trades_df.columns:
        trades_df = trades_df.drop(columns=['Unnamed: 0'])
    
    # 2. Standardize lowercase names
    trades_df.columns = [c.lower().strip() for c in trades_df.columns]
    
    # 3. Handle the 'One-Asset' rule
    ...
    for asset in assets_with_open_trades:
        ...
        if len(open_positions) > 1:
            # Keep only the newest [0], clean the rest [1:]
            ...
            for idx in legacy_indices:
                ...
                print(f"🧹 PENNY: Fixed Legacy BTC Trade from {trades_df.at[idx, 'timestamp']}")
...
def get_firm_ledger(prices_dict=None):
    ...
    # Load and immediately fix formatting
    ...
    # EMERGENCEY CONSOLE LOG
    print("\n" + "!"*30)
    print("🏛️  PENNY: LEDGER REPAIRED")
    print(f"Active Trades: {len(trades_df[trades_df['result'] == 'OPEN'])}")
...
def calculate_unrealized(trades_df, prices_dict):
    """Sentinel View Fixer."""
    # Ensure lowercase to prevent the Sentinel from 'losing' data
    trades_df.columns = [c.lower().strip() for c in trades_df.columns]
